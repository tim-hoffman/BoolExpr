 ...
 * @see http://www.javaspecialists.eu/archive/Issue161.html
 *
 * @param <E>
 */
public class EnumBuster<E extends Enum<E>> {
     ...
     * parameters. The additionalTypes is used to match the constructor
     * accurately.
     *
     * @param value
    ...
    /**
     * This method adds the given enum into the array inside the enum class. If
     * the enum already contains that particular value, then the value is
     * overwritten with our enum. Otherwise it is added at the end of the array.
     *
     * In addition, if there is a constant field in the enum class pointing to
     * an enum with our value, then we replace that with our enum instance.
     *
     * The ordinal is either set to the existing position or to the last value.
     *
     * Warning: This should probably never be called, since it can cause
     * permanent changes to the enum values. Use only in extreme conditions.
     *
    ...
    /**
     * Undo the state right back to the beginning when the EnumBuster was
     * created.
     */
    public void restore() {
        while (undo()) {
    ...
    /**
     * The only time we ever add a new enum is at the end. Thus all we need to
     * do is expand the switch map arrays by one empty slot.
     */
