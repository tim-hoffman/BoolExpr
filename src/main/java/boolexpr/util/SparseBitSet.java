 ...
 *  the Apache-2.0 License) and originally including the following notice:
...
/**
 * This class implements a set of bits that grows as needed. Each bit of the bit
 * set represents a <code>boolean</code> value. The values of a
 * <code>SparseBitSet</code> are indexed by non-negative integers. Individual
 * indexed values may be examined, set, cleared, or modified by logical
 * operations. One <code>SparseBitSet</code> or logical value may be used to
 * modify the contents of (another) <code>SparseBitSet</code> through logical
 * <b>AND</b>, logical <b>inclusive OR</b>, logical <b>exclusive OR</b>, and
 * <b>And NOT</b> operations over all or part of the bit sets.
 * <p>
 ...
 * After the last set bit of a <code>SparseBitSet</code>, any attempt to find a
 * subsequent bit (<i>nextSetBit</i>()), will return an value of &minus;1. If an
 * attempt is made to use <i>nextClearBit</i>(), and all the bits are set from
 * the starting position of the search to the bit labelled
 ...
 * <code>SparseBitSet</code> will result in a <code>NullPointerException</code>.
 * <p>
 ...
 * @author Timothy Hoffman (optimized and added some functionality)
 */
public class SparseBitSet implements Cloneable, Serializable, Iterable<Integer> {
    ...
    /*  My apologies for listing all the additional authors, but concepts, code,
        ...
        deep. Again, to change this might be a large amount of work. On the
        ...
        Whenever possible, a level 3 block that contains no bits (all the words
        are zero, is discarded, and its reference is replaced by a null value.
     ...
     *
     * @see #setStringCompaction(int)
     */
    protected static int compactionCount = 2; // Note: this is not final!
     ...
     * within that word (where bit position 0 refers to the least significant
     * bit and 63 refers to the most significant bit).
     * <p>
     ...
     * length of the bit set, i.e., the index of the last possible bit, plus
     * one. Note: this is not the value returned by <i>length</i>().
     *
     * @see #resize(int)
     * @see #length()
     */
    ...
    /**
     * The label (index) of a bit in the bit set is essentially broken into 4
     * "levels". Respectively (from the least significant end), level4, the
     * address within word, the address within a level3 block, the address
     * within a level2 area, and the level1 address of that area within the set.
     *
    ...
    /**
     * MASK3 is the mask to extract the LEVEL3 address from a word index (after
     * shifting by SHIFT3).
     */
     ...
     * That is, bits per word by words per level3 block by blocks per level2
     * area.
     */
    protected static final int UNIT = LENGTH2 * LENGTH3 * LENGTH4;

    /**
     * MASK2 is the mask to extract the LEVEL2 address from a word index (after
     * shifting by SHIFT3 and SHIFT2).
     */
    ...
    /*  Programming notes:
        ...
        the scanning and optimization logic in one place, and the Strategies
        are reasonably simple (see the definition of AbstractStrategy for a 
        discussion of the tasks that must be defined therein).
    ...
    /**
     * Constructor for a new (sparse) bit set. All bits initially are
     * effectively <code>false</code>. This is a internal constructor that
     * collects all the needed actions to initialise the bit set.
     * <p>
     * The capacity is taken to be a <i>suggestion</i> for a size of the bit
     * set, in bits. An appropriate table size (a power of two) is then
     * determined and used. The size will be grown as needed to accommodate any
     * bits addressed during the use of the bit set.
     *
     * @param nbits a size in terms of bits
     *
     * @exception NegativeArraySizeException if the specified initial size is
     *                                       negative
     * @since 1.6
     */
    protected SparseBitSet(int nbits) throws NegativeArraySizeException {
        /* Array size is computed based on this being a capacity given in bits. */
        if (nbits < 0) { //Capacity can't be negative
            throw new NegativeArraySizeException("(requested capacity=" + nbits + ") < 0");
        }
    ...
    /**
     * Constructs an empty bit set with the default initial size. Initially all
     * bits are effectively <code>false</code>.
     *
     * @since 1.6
     */
    public SparseBitSet() {
        this(1);//Requesting 1 bit, will actually get UNIT number of bits
    }

    public static SparseBitSet make(int... elems) {
     ...
     * @see #toString()
     */
    public static void setStringCompaction(int count) {
        compactionCount = count;
    }

    /**
     * Performs a logical <b>AND</b> of the addressed target bit with the
     * argument value. This bit set is modified so that the addressed bit has
     * the value <code>true</code> if and only if it both initially had the
     * value <code>true</code> and the argument value is also <code>true</code>.
     *
     * @param i     a bit index
     ...
     * modified so that each bit in it has the value <code>true</code> if and
     * only if it both initially had the value <code>true</code> and the
     * corresponding bit in the bit set argument also had the value
     * <code>true</code>. Outside the range, this set is not changed.
     *
     ...
     *                                      equal to Integer.MAX_VALUE, or
     *                                      <code>j</code> is negative, or
     *                                      <code>i</code> is larger than
     *                                      <code>j</code>
     * @since 1.6
     */
    ...
    /**
     * Performs a logical <b>AND</b> of the two given
     * <code>SparseBitSet</code>s. The returned <code>SparseBitSet</code> is
     * created so that each bit in it has the value <code>true</code> if and
     * only if both the given sets initially had the corresponding bits
    ...
    /**
     * Performs a logical <b>AND</b> of all given <code>SparseBitSet</code>s.
     * The returned <code>SparseBitSet</code> is created so that each bit in it
     * has the value <code>true</code> if and only if all of the given sets
     ...
     * @param input
     *
     * @return a new SparseBitSet representing the <b>AND</b> of all sets given
     */
    public static SparseBitSet and(Iterable<SparseBitSet> input) {
        Iterator<SparseBitSet> itr = input.iterator();
        assert itr.hasNext();//must have at least one
        SparseBitSet result = itr.next().clone();
        while (itr.hasNext()) {
            result.and(itr.next());
        }
        return result;
     ...
     * argument value. This bit set is modified so that the addressed bit has
     * the value <code>true</code> if and only if it both initially had the
     * value <code>true</code> and the argument value is <code>false</code>.
     *
     * @param i     a bit index
     ...
     * modified so that each bit in it has the value <code>true</code> if and
     * only if it both initially had the value <code>true</code> and the
     * corresponding bit in the bit set argument has the value
     * <code>false</code>. Outside the range, this set is not changed.
     *
     * @param i index of the first bit to be included in the operation
     * @param j index after the last bit to included in the operation
     ...
     * in it has the value <code>true</code> if and only if the corresponding
     * bit in the <code>SparseBitSet</code> of the first is set, and that same
     * corresponding bit is not set in the <code>SparseBitSet</code> of the
     * second argument.
     *
     * @param a a SparseBitSet
     * @param b another SparseBitSet
     *
     * @return a new SparseBitSet representing the <b>AndNOT</b> of the two sets
     *
     * @since 1.6
     */
     ...
     * <p>
     * Note: the actual space allocated to the clone tries to minimise the
     * actual amount of storage allocated to hold the bits, while still trying
     * to keep access to the bits being a rapid as possible. Since the space
    ...
    public SparseBitSet clone() {
        try {
            ...
            result.bits = null;
            result.resize(1);
            result.cache = new Cache();//reset the cache
     ...
     * <pre>((SparseBitSet)obj).get(i) == this.get(i)</pre> must be true.
     *
     ...
     * <code>true</code> if the bit with the index <code>i</code> is currently
     * set in this <code>SparseBitSet</code>; otherwise, the result is
     * <code>false</code>.
     *
     ...
     * <code>SparseBitSet</code> from <code>i</code> (inclusive) to
     * <code>j</code> (exclusive).
     *
    ...
    public SparseBitSet get(int i, int j) throws IndexOutOfBoundsException {
        SparseBitSet result = new SparseBitSet(j);
     ...
     * @return
     */
    public boolean containsAll(SparseBitSet other) {
        //NOTE: There are 2 approaches to computing this. According to a runtime
        //  comparison of the approaches (performed on March 11, 2020) when the
        //  statistics for 'other' have already been computed, the loop-based
        //  approach is generally faster (with 95% accuracy), otherwise, the
        //  strategy-based approach is faster (with 92% accuracy). Thus, we
        //  choose the approach to take accordingly.
        if (other.cache.hash == 0) {
            return containsAll_Strat(other);
        } else {
            return containsAll_Loop(other);
        }
//        boolean thisCached = this.cache.hash != 0;
//        boolean otherCached = other.cache.hash != 0;
//
//        long start, elapLoop = 0, elapStrat = 0;//TODO:TEMP:DEBUG
//        start = System.nanoTime();
//        boolean r1 = containsAll_Loop(other);
//        elapLoop += System.nanoTime() - start;
//        start = System.nanoTime();
//        boolean r2 = containsAll_Strat(other);
//        elapStrat += System.nanoTime() - start;
//        assert r1 == r2;
//
//        {
//            System.out.println("[FINDME] 1st itr winner," + (elapStrat < elapLoop ? "strat" : "loop")
//                    + "," + otherCached + "," + thisCached
//                    + "," + (other.cache.hash != 0 ? other.cardinality() : -1)
//                    + "," + (this.cache.hash != 0 ? this.cardinality() : -1)
//                    + "," + elapLoop + "," + elapStrat);
//            //OBS: 
//            // Q: When other is cached, which is typically faster?
//            // A: Loop is faster 99.5% of the time (averaging 670ns faster) and
//            //      the other times, strat averages 1766ns faster.
//            // CONCLUSION: when other is cached, do the loop
//            // Q: When other is NOT cached, which is faster?
//            // A: Strat is faster (or equal) 91.3% of the time (averaging 2304ns
//            //      faster) and the other times, loop averages 1475ns faster.
//            //      If  strat is chosen in all cases it only takes 85600ns 
//            //      (4.5%) longer than choosing the minimum in all cases.
//        }
//
//        if (false) {
//            //OBS: generally around 32ns/iter for loop and 253ns/iter for stat
//            //
//            //Treating the above as a warmup, compute the results many times
//            //  and compare the )
//            final int numIter = 200_000;
//            final java.util.Random r = new java.util.Random();
//            elapLoop = 0;
//            elapStrat = 0;
//            for (int i = 0; i < numIter; i++) {
//                if (r.nextBoolean()) {//randomize the order of the two tests
//                    //Do Loop then Strat
//                    start = System.nanoTime();
//                    r1 = containsAll_Loop(other);
//                    elapLoop += System.nanoTime() - start;
//
//                    start = System.nanoTime();
//                    r2 = containsAll_Strat(other);
//                    elapStrat += System.nanoTime() - start;
//                } else {
//                    //Do Strat then Loop
//                    start = System.nanoTime();
...
//                assert r1 == r2;
//            }
//            System.out.println("[containsAll: ns/iter] loop=" + (elapLoop / numIter) + ", strat=" + (elapStrat / numIter));
//            if (elapStrat < elapLoop) {
//                System.out.println("[FINDME] strategy was faster!");
//            }
//        }
//        return r1;
    }

    protected boolean containsAll_Loop(SparseBitSet other) {
        if (this == other) {
            return true; // Identity
        }
        //Traverse 'other' to ensure that all values are in 'this'.
        //NOTE: even though traversal of SparseBitSet can be slow with large
        //  and/or sparse values, it's likely faster than clone+and+equals.
        //NOTE: using minSetBit() does actually compute and cache statistics
        //  so I would be free to also use the cardinality if needed.
        for (int i = other.minSetBit(); i >= 0; i = other.nextSetBit(i + 1)) {
            if (!this.get(i)) {
                return false;
            }
        }
    ...
    protected boolean containsAll_Strat(SparseBitSet other) {
        if (this == other) {
            return true; // Identity
        }
        //TODO: I'm not certain if it's possible to short circuit the strategy
        //  somehow if the containment result ever becomes false.
        ContainsAllStrategy strat = new ContainsAllStrategy();
        setScanner(0, Math.max(bitsLength, other.bitsLength), other, strat);
        return strat.result;
    }
     ...
     * such a manner that bit <code>i</code> is set in the
     * <code>SparseBitSet</code> (for nonnegative values of  <code>i</code>) if
     * and only if the expression
     * <pre>
     *  ((i&gt;&gt;6) &lt; bits.length) &amp;&amp; ((bits[i&gt;&gt;6] &amp; (1L &lt;&lt; (bit &amp; 0x3F))) != 0)
     * </pre> is true. Then the following definition of the
     * <code>hashCode</code> method would be a correct implementation of the
     * actual algorithm:
     * <pre>
     *  public int hashCode()
     *  {
     ...
     *          hash ^= bits[i] * (i + 1);
     *      return (int)((h &gt;&gt; 32) ^ h);
     *  }</pre> Note that the hash code values change if the set of bits is
     * altered.
     *
     ...
     * (exclusive) set to <code>true</code> that are also set to
     * <code>true</code> in the same range of this <code>SparseBitSet</code>.
     *
     * @param i index of the first bit to include
     * @param j index after the last bit to include
    ...
    /**
     * Returns true if the specified <code>SparseBitSet</code> has any bits set
     * to <code>true</code> that are also set to <code>true</code> in this
     * <code>SparseBitSet</code>.
     *
    ...
    public boolean intersects(SparseBitSet b) {
        return intersects(0, Math.max(bitsLength, b.bitsLength), b);
    }
    ...
    public int nextClearBit(int i) {
        ...
        long nword = ~0L << i;
        final long[][][] bits = this.bits;//local reference to the field for speed
     ...
     *
     * <pre>
     *  for( int i = sbbits.minSetBit(); i &gt;= 0; i = sbbits.nextSetBit(i+1) )
     *  {
     ...
     * argument value. This bit set is modified so that the addressed bit has
     * the value <code>true</code> if and only if it both initially had the
     * value <code>true</code> or the argument value is <code>true</code>.
     *
     * @param i     a bit index
     ...
     * within the range a bit in it has the value <code>true</code> if and only
     * if it either already had the value <code>true</code> or the corresponding
     * bit in the bit set argument has the value <code>true</code>. Outside the
     * range this set is not changed.
     *
     * @param i index of the first bit to be included in the operation
     * @param j index after the last bit to included in the operation
     ...
     * the value <code>true</code> if and only if it had the value
     * <code>true</code> in either the first argument or the second argument.
     *
     * @param a a SparseBitSet
     * @param b another SparseBitSet
     *
    ...
    /**
     * Performs a logical <b>OR</b> of all given <code>SparseBitSet</code>s. The
     * returned <code>SparseBitSet</code> is created so that a bit in it has the
     * value <code>true</code> if and only if it had the value <code>true</code>
     * in any of the {@link SparseBitSet} given.
     *
     *
     * @param input
     *
     * @return a new SparseBitSet representing the <b>OR</b> of all sets given
     */
    public static SparseBitSet or(Iterable<SparseBitSet> input) {
        Iterator<SparseBitSet> itr = input.iterator();
        assert itr.hasNext();//must have at least one
        SparseBitSet result = itr.next().clone();
        while (itr.hasNext()) {
    ...
    /**
     * Sets the bits at the specified indices.
     *
     * @param bits indices of bits to set
     *
     * @exception IndexOutOfBoundsException if any specified index is negative
     *                                      or equal to Integer.MAX_VALUE
     * @since 1.6
     */
    public void setAll(Iterable<Integer> bits) {
        for (Integer i : bits) {
            set(i);
        }
    }
    ...
    /**
     * Sets the bits from the specified <code>i</code> (inclusive) to the
     * specified <code>j</code> (exclusive) to the specified value.
     *
     * @param i     index of the first bit to be set
     ...
     * set bit).
     *
     * @return the number of bits (true and false) nominally in this bit set at
     *         this moment
     *
     * @since 1.6
     */
    ...
    /**
     * Returns the index of the first bit that is set to <code>true</code>. If
    ...
    public int minSetBit() {
        statisticsUpdate();
        return cache.minValue;
    }

    /**
     * Returns the index of the last bit that is set to <code>true</code>. If no
    ...
    public int maxSetBit() {
        statisticsUpdate();
        int size = cache.size;
        return size == 0 ? -1 : (size - 1 + cache.minValue);
    }

    /**
     * Convenience method for statistics if the individual results are not
     * needed.
     *
    ...
    /**
     * Determine, and create a String with the bit set statistics. The
     * statistics include: Size, Length, Cardinality, Total words (<i>i.e.</i>,
     * the total number of 64-bit "words"), Set array length (<i>i.e.</i>, the
     * number of references that can be held by the top level array), Level2
     * areas in use, Level3 blocks in use, Level2 pool size, Level3 pool size,
    ...
    /**
     * Calls {@link #toString(int)} with the current static
     * {@code compactionCount} value.
     *
    ...
    public String toString() {
        return toString(compactionCount);
    }

    /**
     * Returns a string representation of this bit set. For every index for
     * which this <code>SparseBitSet</code> contains a bit in the set state, the
     * decimal representation of that index is included in the result. Such
     * indices are listed in order from lowest to highest. If there is a
     * subsequence of set bits longer than the {@code useCompactionCount} value
     * given, the subsequence is represented by the value for the first and the
     * last values, with ".." between them. The individual bits, or the
     * representation of sub-sequences are separated by ",&nbsp;" (a comma and a
     * space) and surrounded by braces, resulting in a compact string showing (a
     * variant of) the usual mathematical notation for a set of integers.
     * <br>
     * Example (with the default value of 2 for subsequences):
     * <pre>
     *      SparseBitSet drPepper = new SparseBitSet();
     * </pre> Now <code>drPepper.toString()</code> returns "<code>{}</code>".
     * <br>
     * <pre>
     *      drPepper.set(2);
     * </pre> Now <code>drPepper.toString()</code> returns "<code>{2}</code>".
     * <br>
     * <pre>
     *      drPepper.set(3, 4);
     *      drPepper.set(10);
     * </pre> Now <code>drPepper.toString()</code> returns
     * "<code>{2..4, 10}</code>".
     * <br>
     * This method is intended for diagnostic use (as it is relatively expensive
     * in time), but can be useful in interpreting problems in an application's
     * use of a <code>SparseBitSet</code>.
     *
     * @param useCompactionCount
     *
     * @return a String representation of this SparseBitSet
     *
     * @see #setStringCompaction(int)
     * @since 1.6
     */
    public String toString(int useCompactionCount) {
     ...
     * argument value. This bit set is modified so that the addressed bit has
     * the value <code>true</code> if and only one of the following statements
     * holds:
     * <ul>
     * <li>The addressed bit initially had the value <code>true</code>, and the
     * value of the argument is <code>false</code>.
     * <li>The bit initially had the value <code>false</code>, and the value of
     * the argument is <code>true</code>.
     * </ul>
     *
     * @param i     a bit index
     * @param value a boolean value to <b>XOR</b> with that bit
     *
     * @exception java.lang.IndexOutOfBoundsException if the specified index is
    ...
    /**
     * Performs a logical <b>XOR</b> of the two given
     * <code>SparseBitSet</code>s. The resulting bit set is created so that a
     * bit in it has the value <code>true</code> if and only if one of the
     ...
     * the set array are discarded. If the new array is bigger, it is filled
     * with nulls.
     *
    ...
    /**
     * Scans over the bit set (and a second bit set if part of the operation)
     * are all performed by this method. The properties and the operation
     * executed are defined by a given strategy, which must be derived from the
     ...
     */
    protected final void setScanner(int i, int j, SparseBitSet b, AbstractStrategy op)
            throws IndexOutOfBoundsException {
        ...
        /*  Initialize the local copies of the counts of blocks and areas */
        int a2CountLocal = 0;
        int a3CountLocal = 0;
        if (i != j) {
            ...
            /*  Initialize whether there is a partial first block.  */
            boolean notFirstBlock = u == 0 && um == ~0L;

            /*  The first level2 cannot be judged empty if not being scanned
            from the beginning. */
            boolean a2IsEmpty = u2 == 0; //  Presumption
            while (i < j) {
                /*  Determine if there is a level2 area in both the a and the b
                set, and if so, set the references to these areas. */
                long[][] a2 = null;
                ...
                if ((!haveA2 && !haveB2 && f_op_f_eq_f
                    ...
                    while (u2 != limit2) {
                        ...
                        if ((!haveA3 && !haveB3 && f_op_f_eq_f
                            ...
                            //A spare level 3 block for when a target block is
                            //  needed, and there is not already one in the bit
                            //  set. If non-zero values are placed into this
                            //  block, it is moved to the resulting set.
     ...
     * @since 1.6
     */
    private static boolean isZeroBlock(long[] a3) {
        for (long word : a3) {
            if (word != 0L) {
                return false;
            }
     ...
     * the bit set (size, length, cardinality, hashCode, etc.) are computed.
     * Level arrays that are empty (i.e., all zero at level 3, all null at level
     * 2) are replaced by null references, ensuring a normalized representation.
     *
     * @since 1.6
     */
     ...
     *                                    inconsistent
     *
     * @serialData The default data is emitted, followed by the <i>length</i> of
     * the set (the position of the last bit), followed by the
     * <i>cache.count</i> value (an <code>int</code>, the number of
     * <code>int-&gt;long</code> pairs needed to describe the set), followed by
     * the index (<code>int</code>) and word (<code>long</code>) for each
     * <code>int-&gt;long</code> pair. The mappings need not be emitted in any
     * particular order. This is followed by the <i>hashCode</i> for the set
     * that can be used as an integrity check when the bit set is read back.
     *
     * @since 1.6
     */
    ...
    public Iterator<Integer> iterator() {
        return new Iterator<Integer>() {
            private int nextVal = minSetBit();
            private Integer lastReturnedVal = null;
            ...
            public Integer next() {
                if (!hasNext()) {
                    throw new NoSuchElementException("This iterator does not have any more elements");
                }
                lastReturnedVal = nextVal;
                nextVal = nextSetBit(nextVal + 1);
                return lastReturnedVal;
            }

            @Override
            public void remove() {
                if (lastReturnedVal == null) {
                    throw new IllegalStateException();
                }
                clear(lastReturnedVal);
                lastReturnedVal = null;
            }
        };
    }
     ...
     * created by the <i>statistics</i>() method. The values of the
     * corresponding statistics are <code>int</code>s, except for the loadFactor
     * and Average_chain_length values, which are <code>float</code>s.
     * <p>
    ...
    public static enum Statistics {
        ...
        Length, // 1
        /**
         * The cardinality of the bit set, as give by the <i>cardinality</i>()
         * method.
         */
        Cardinality, // 2
        /**
    ...
    protected static class Cache {
        ...
        /**
         * <i>hash</i> is updated by the <i>statisticsUpdate</i>() method. If
         * the <i>hash</i> value is zero, it is assumed that <b><i>all</i></b>
         * the cached values are stale, and must be updated.
         */
        protected transient int hash = 0;

        /**
         * <i>size</i> is updated by the <i>statisticsUpdate</i>() method. If
         * the <i>hash</i> value is zero, it is assumed the all the cached
         * values are stale, and must be updated.
         */
        protected transient int size = 0;

        /**
         * <i>minValue</i> is updated by the <i>statisticsUpdate</i>() method.
         * If the <i>hash</i> value is zero, it is assumed the all the cached
         * values are stale, and must be updated.
         */
        protected transient int minValue = -1;

        /**
         * <i>cardinality</i> is updated by the <i>statisticsUpdate</i>()
         * method. If the <i>hash</i> value is zero, it is assumed the all the
         * cached values are stale, and must be updated.
         */
        protected transient int cardinality = 0;

        /**
         * <i>length</i> is updated by the <i>statisticsUpdate</i>() method. If
         * the <i>hash</i> value is zero, it is assumed the all the cached
         * values are stale, and must be updated.
         */
        ...
        /**
         * <i>count</i> is updated by the <i>statisticsUpdate</i>() method. If
         * the <i>hash</i> value is zero, it is assumed the all the cached
         * values are stale, and must be updated.
         */
         ...
         * <i>hash</i> value is must be zero for all values to be updated.
         */
        protected transient int a3Count = 0;
    }
     ...
     * <i>setScanner</i>() method of the main <code>SparseBitSet</code> class.
     *
     * @see SparseBitSet#setScanner(int i, int j, SparseBitSet b,
     * AbstractStrategy op)
     */
    protected static abstract class AbstractStrategy {
         ...
         * required. Corresponds to the top-left entry in the logic diagram for
         * the operation being 0. For all the defined actual logic operations
         * ('and', 'andNot', 'or', and 'xor', this will be true, because for all
         * these, "false" op "false" = "false".
         */
        static final int F_OP_F_EQ_F = 0x1;

        /**
         * If when level2 areas or level3 areas from the this set are null will
         * require that area or block to remain null, irrespective of the value
         * of the matching structure from the other set, then this property is
         * required. Corresponds to the first row in the logic diagram being all
         * zeros. For example, this is true for 'and' as well as 'andNot', and
         * for 'clear', since false" & "x" = "false", and "false" &! "x" =
         * "false".
         */
         ...
         * matching area or block in this set to be left as it is, then this
         * property is required. Corresponds to the first column of the logic
         * diagram being equal to the left hand operand column. For example,
         * this is true for 'or', 'xor', and 'andNot', since for all of these
         * "x" op "false" = "x".
         */
         ...
         * particular use, an instance is (re-)started by calling this method.
         * It is passed the reference to the other bit set (usually to allow a
         * check on whether it is null or not, so as to simplify the
         * implementation of the <i>block</i>() method.
         *
         ...
         * @since 1.6
         */
        protected abstract void start(Cache cache, SparseBitSet b);

        /**
         * Deal with a scan that includes a partial word within a level3 block.
         * All that is required is that the result be stored (if needed) into
         * the given a set block at the correct position, and that the operation
         * only affect those bits selected by 1 bits in the mask.
         *
         ...
         * @param b3   the (nominal) level3 block from the <i>b</i> set (not
         *             null).
        ...
        /**
         * Deals with a part of a block that consists of whole words, starting
         * with the given first index, and ending with the word before the last
         * index. For the words processed, the return value should indicate
         * whether all those resulting words were zero, or not.
         *
         * @param base the base index of the block (to be used if needed)
         ...
         *             null).
         *
         * @return true if the words scanned within the level3 block were all
         *         zero
         *
         * @since 1.6
         */
        ...
        /**
         * This is called to finish the processing started by the strategy (if
         * there needs to be anything done at all).
         *
     ...
     * <i>b</i>, then the unmatched virtual "entries" of the <i>b</i> set
     * (beyond the actual length of <i>b</i>) corresponding to these are all
     * false, hence the result of the "and" operation will be to make all these
     * entries in this set to become false--hence just remove them, and then
     * scan only those entries that could match entries in the bit set<i>b</i>.
     * This clearing of the remainder of the <i>a</i> set is accomplished by
     * selecting both
    ...
    protected static class AndStrategy extends AbstractStrategy {
        ...
        @Override
        //  AndStrategy
        protected void start(Cache cache, SparseBitSet b) {
            if (b == null) {
    ...
    protected static class EqualsStrategy extends AbstractStrategy {
        ...
        protected void start(Cache cache, SparseBitSet b) {
            ...
            result = true;
            //does not change the content of the set, hence no hash reset
        }
        ...
        protected boolean word(int base, int u3, long[] a3, long[] b3, long mask) {
            ...
            result = result && ((word & mask) == (b3[u3] & mask));
            return word == 0L;
        }
        ...
        protected boolean block(int base, int u3, int v3, long[] a3, long[] b3) {
            ...
            for (int w3 = u3; w3 != v3; ++w3) {
                ...
                result = result && (word == b3[w3]);
                isZero = isZero && (word == 0L);
            }
            return isZero;
        }
    }
    ...
    //-----------------------------------------------------------------------------
    /**
     * TODO: "A containsAll B" implemented as "(A&B)==B"
     */
    protected static class ContainsAllStrategy extends AbstractStrategy {
        ...
        //  ContainsAllStrategy
        protected int properties() {
            //NOTE: Adding X_OP_F_EQ_X speeds up the process by a factor of 2-3
            //  times in near-max size SparseBitSet samples.
        ...
        //  ContainsAllStrategy
        protected boolean word(int base, int u3, long[] a3, long[] b3, long mask) {
            final long wordA = a3[u3];
            if (result) {//short-circuit: if result is false, it cannot become true
                final long maskAndB = b3[u3] & mask;
                result = (wordA & maskAndB) == maskAndB;
            }
            return wordA == 0L;//check orig value to prevent modification
        }
        ...
        protected boolean block(int base, int u3, int v3, long[] a3, long[] b3) {
            boolean isZero = true; //  Presumption
            for (int w3 = u3; w3 != v3; ++w3) {
                final long wordA = a3[w3];
                if (result) {//short-circuit: if result is false, it cannot become true
                    final long wordB = b3[w3];
                    result = (wordA & wordB) == wordB;
                }
                isZero = isZero && (wordA == 0L);//check orig value to prevent modification
            }
            return isZero;
        }
    }
    ...
    //-----------------------------------------------------------------------------
    /**
     * Intersect has a true result if any word in the <i>a</i> set has a bit in
     * common with the <i>b</i> set. During the scan of the <i>a</i> set blocks
     * (and areas) that are all zero may be replaced with empty blocks and areas
     * (null references), but the value of the set is not changed (which is why
     * X_OP_F_EQ_F is not selected, since this would cause parts of the <i>a</i>
     * set to be zero-ed out).
     *
     * <pre>
     * intersect| 0 1
     *         0| 0 0
    ...
    protected static class IntersectsStrategy extends AbstractStrategy {
        ...
        protected boolean word(int base, int u3, long[] a3, long[] b3, long mask) {
            final long word = a3[u3];
            result = result || ((word & b3[u3] & mask) != 0L);
            return word == 0L;
        }
     ...
     * where the <i>b</i> set is one, the <i>a</i> becomes one. If both sets
     * have zeros in corresponding places, a zero results. Whole blocks or areas
     * that are or become zero are replaced by null arrays.
     * <p>
     * If level1 of the <i>a</i> set is longer than level1 of the bit set
     * <i>b</i>, then the unmatched entries of the <i>a</i> set (beyond the
     * actual length of <i>b</i>) corresponding to these remain unchanged.
     *
     * <pre>
     *   or| 0 1
     *    0| 0 1
     ...
     */
    protected static class UpdateStrategy extends AbstractStrategy {

        /**
         * Keep reference to the cache from the #start(..) method so that the
         * #finish(..) method can update it.
         */
        protected transient Cache cache;
        ...
        /**
         * Working space for find the size and length of the bit set. Holds copy
         * of the first non-empty word in the set.
         */
        ...
        /**
         * Working space for find the size and length of the bit set. Holds a
         * copy of the last non-empty word in the set.
         */
        ...
        /**
         * Working space for keeping count of the number of non-zero words in
         * the bit set. Holds the current state of the computation of the count.
         * This value is ultimately transferred to the Cache object.
         *
         * @see SparseBitSet.Cache
         */
        ...
        /**
         * Working space for counting the number of non-zero bits in the bit
         * set. Holds the current state of the computation of the
         * cardinality.This value is ultimately transferred to the Cache object.
         *
         * @see SparseBitSet.Cache
         */
        ...
        protected void start(Cache cache, SparseBitSet b) {
            this.cache = cache;
            this.hash = 1234L; // Magic number
            this.wMin = -1; // index of first non-zero word
            this.wordMin = 0L; // word at that index
            this.wMax = 0; // index of last non-zero word
            this.wordMax = 0L; // word at that index
            this.count = 0; // count of non-zero words in whole set
            this.cardinality = 0; // count of non-zero bits in the whole set
        }
        ...
        protected void finish(int a2Count, int a3Count) {
            ...
            if (wMin < 0) {
                ...
                int w3 = wMin & MASK3;
                int w2 = (wMin >> SHIFT2) & MASK2;
                int w1 = wMin >> SHIFT1;
                cache.minValue = (((w1 << SHIFT1) + (w2 << SHIFT2) + w3) << SHIFT3) + Long.numberOfTrailingZeros(wordMin);
            }
            //NOTE: 'cache.hash' is non-zero as long as 'hash' is not 0 or -1
            cache.hash = (int) ((hash >> Integer.SIZE) ^ hash);
        }

        /**
         * This method does the accumulation of the statistics. It must be
         * called in sequential order of the words in the set for which the
         * statistics are being accumulated, and only for non-null values of the
         * second parameter.
         *
         * Two of the values (a2Count and a3Count) are not updated here, but are
         * done in the code near where this method is called.
         *
    ...
    //--------------------------------------------------------------------------
    //[Tim Hoffman]: Most of the strategies have been made static in order to
    //  conserve memory when millions of SparseBitSets are created. They are
    //  safe to be made static (and thus shared across all instances) because
    //  they keep no internal state (unlike Equals, Update, and Intersects).
    //  I had to make the *Strategy classes static (and add the Cache as a
    //  parameter to the two methods that actually use it) in order to do this.
    //  This should also improve memory usage. In fact, a newer version of this
    //  class that I found on line did nearly the same thing as I did here.
    //  Additionally, the strategies that do have an internal state have been
    //  removed as fields of 'this' and just created on-demand when needed
    //  in order to make non-modifying operations thread-safe.
    //
    //TODO: ideally, the Cache updates (specifically statistics update) should
    //  also be thread-safe!
    //
    /**
